...
def load_data_synth(universe: dict = UNIVERSE, start: str = "2003-01-01",
                    end: str | None = None, seed: int = 42) -> dict:
    """Generate synthetic daily OHLC for offline/sandboxed runs.

    Each pair gets a GBM-ish daily close with regime-shifting drift and vol,
    plus realistic intraday ranges. Identical to a live feed in shape so the
    downstream pipeline is exercised end-to-end. Use --source yf for real data.
    """
    rng = np.random.default_rng(seed)
    end = end or pd.Timestamp.today().normalize().strftime("%Y-%m-%d")
    idx = pd.bdate_range(start=start, end=end)
    out = {}
    for i, (name, _) in enumerate(universe.items()):
        # JPY pairs trade at ~100, EUR/GBP majors near ~1.2, NOK/SEK near ~9
        if "JPY" in name:
            level = 110.0
        elif name in ("USDNOK", "USDSEK"):
            level = 8.5
        else:
            level = 1.20
        # base daily vol ~0.6% with occasional vol-clustered shocks
        n = len(idx)
        sub = rng.normal(0.0, 1.0, size=n)
        # vol regimes (slow-varying)
        vol = 0.005 + 0.004 * (np.sin(np.linspace(0, 6 * np.pi + i, n)) + 1) / 2
        drift = 0.00005 * np.sin(np.linspace(0, 4 * np.pi + i * 0.7, n))
        ret = drift + vol * sub
        close = level * np.exp(np.cumsum(ret))
        # synthesise OHLC: open = prior close + small gap; H/L bracket via intraday range
        open_ = np.empty(n); open_[0] = close[0]
        open_[1:] = close[:-1] * (1 + rng.normal(0, 0.0005, size=n - 1))
        intraday_range = np.abs(rng.normal(0, 1, size=n)) * vol * close
        high = np.maximum(open_, close) + 0.5 * intraday_range
        low  = np.minimum(open_, close) - 0.5 * intraday_range
        df = pd.DataFrame({"Open": open_, "High": high, "Low": low, "Close": close}, index=idx)
        out[name] = df
        print(f"  {name}: {len(df)} bars  {df.index[0].date()} -> {df.index[-1].date()}  (synthetic)")
    return out
...
def sig_trend_pullback(d: pd.DataFrame, look=5):
    ...
    touched_ma_dn = (d["Low"] <= d["ma20"]).rolling(look).max().astype(bool)
    ...
    for dt in d.index[long_.fillna(False)]:
        row = d.loc[dt]
        stop = d["Low"].loc[:dt].iloc[-look:].min()
        if pd.notna(stop) and stop < row["Close"]:
...
def sig_failed_breakout(d: pd.DataFrame, window=3, buf_atr=0.1):
    out = []
    broke_hi = (d["High"] > d["dc_hi"]).rolling(window).max().astype(bool)
    broke_lo = (d["Low"]  < d["dc_lo"]).rolling(window).max().astype(bool)
    back_in_short = broke_hi & (d["Close"] < d["dc_hi"])
...
class TradeResult:
    ...
    target_day: dict = field(default_factory=dict)
...
def backtest(data_feat: dict, signals=SIGNALS, max_days=MAX_HORIZON, conservative=True):
    """Run every signal over every pair. Entry = next bar's open."""
    trades = []
        ...
        for name, fn in signals.items():
            ...
            for _, e in today.iterrows():
                ...
                rows.append(dict(pair=pair, dir=e["direction"], signal=name,
                                 ...
                                 **tgts, EV_R=s.get("EV_R") if hasattr(s, "get") else None,
                                 hit=s.get("hit_rate") if hasattr(s, "get") else None,
                                 P3R=s.get("P(3R<stop)") if hasattr(s, "get") else None,
                                 regime=d["regime"].iloc[-1]))
...
def run(start="2003-01-01", target=3, source="yf"):
    print(f"Loading data (source={source}) ...")
    raw = load_data_yf(start=start) if source == "yf" else load_data_synth(start=start)
    if not raw:
    ...
    rep = report(trades, target)
    print(rep.to_string())
    print("\n=== DECADE STABILITY ===")
    print(decade_split(trades, target).to_string())
    print("\n=== TODAY'S BLOTTER ===")
    bl = live_blotter(feat, rep, target=target)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--source", choices=["yf", "synthetic"], default="yf",
                    help="Data source: 'yf' for Yahoo (default), 'synthetic' for offline GBM data.")
    ap.add_argument("--start", default="2003-01-01")
    ap.add_argument("--target", type=int, default=3)
    args = ap.parse_args()
    run(start=args.start, target=args.target, source=args.source)
